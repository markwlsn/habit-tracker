...
export const register: RequestHandler = async (req, res, next) => { try { res.status(201).json({ data: await auth.register(req.body.email, req.body.password, req.body.name) }); } catch (e) { next(e); } };
